...
def _draw_fit_or_truncate(
    c: canvas.Canvas, text: str, x: float, y: float, max_width: float, base_font: float, draws: list[DrawRecord], page: int
) -> None:
    """일반 필드용: 폰트를 줄여보고, 그래도 안 되면 말줄임표로 자른다."""
    if not text:
        return
    size = _fit_font_size(text, max_width, base_font)
    if size is not None:
    ...
    # 말줄임 — 허용된 필드에서만 호출됨
    size = base_font if base_font < 6 else 6.0
    truncated = text
...
def _draw_license_no(
    c: canvas.Canvas, value: str, draws: list[DrawRecord], footnote_no: int
) -> str | None:
    ...
    if "-" in value:
        ...
        row_height = pos["row_top_y"] - pos["row_bottom_y"]  # reportlab y는 아래가 작으므로 부호 주의
        row_height = abs(row_height)
...
def build_official_overlay(
    ...
    for page_no in range(1, num_pages + 1):
        if page_no == 1:
            ...
            _draw_fit_or_truncate(c, farm_info.get("owner", ""), f["x"], f["y"], f["max_width"], f["font"], draws, 1)

            f = FIELD_POSITIONS["address"]
            _draw_fit_or_truncate(c, farm_info.get("region", ""), f["x"], f["y"], f["max_width"], f["font"], draws, 1)

            f = FIELD_POSITIONS["contact"]
            _draw_fit_or_truncate(c, farm_info.get("contact", ""), f["x"], f["y"], f["max_width"], f["font"], draws, 1)

            f = FIELD_POSITIONS["farm_name"]
            _draw_fit_or_truncate(c, farm_info.get("farm_name", ""), f["x"], f["y"], f["max_width"], f["font"], draws, 1)

            f = FIELD_POSITIONS["farm_area_ha"]
            _draw_fit_or_truncate(
                c, str(farm_info.get("farm_area_ha", "")), f["x"], f["y"], f["max_width"], f["font"], draws, 1
            )

            note = _draw_license_no(c, farm_info.get("license_no", ""), draws, footnote_counter)
            if note:
            ...
            _draw_fit_or_truncate(
                c, ai_analysis.quantity_estimate, f["x"], f["y"], f["max_width"], f["font"], draws, 1
            )

            f = FIELD_POSITIONS["cause_summary"]
            cause_short = ai_analysis.cause_estimate[:15]
            _draw_fit_or_truncate(c, cause_short, f["x"], f["y"], f["max_width"], f["font"], draws, 1)
