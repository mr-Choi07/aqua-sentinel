...
어종별 임계온도(SPECIES_THRESHOLDS)는 아래 근거로 작성했다.
  ...
  - 굴: 개체별 폐사 임계수온을 규정한 학술/공식 자료를 찾지 못했다. 오히려 반대
    방향 증거가 있다 — 굴은 30℃ 이상에서도 생존하는 개체가 보고되어 있고,
    업계·전문가 다수가 "굴 집단폐사는 고수온 자체보다 동반되는 빈산소수괴(산소
    부족 물덩어리)가 주된 원인"이라고 본다(수산인신문 2024, 국립수산과학원 고수온
    내성 품종 개발 관련 보도 참고). 즉 단일 수온 임계치로 굴의 위험도를 판단하는
    모델 자체가 이 어종에는 잘 안 맞을 수 있다는 뜻이라, 근거가 약하다는 걸 알면서
    일반 기준을 잠정 사용한다. 정확히 하려면 임계수온이 아니라 빈산소수괴 속보
    연동이 필요하다(README "현재 범위와 한계" 참고).
  - 참돔: 우럭처럼 28℃/3일 지속이라는 공식 고수온 경보 기준과 연계해서 폐사가
    보도되는 사례가 많다(여수 거문도 참돔·조피볼락 양식장 고수온 피해 보도 등).
    참돔만의 별도 임계수온을 규정한 개체별 학술 자료는 찾지 못했지만, 실제 피해
    사례가 일반 기준과 같은 온도대에서 반복 보고되는 것으로 보아 일반 기준을
    그대로 쓰는 근거가 어느 정도 있다고 판단했다. 저수온(참돔은 하한내성수온
    6.54℃ 부근으로 보고됨)에는 반대로 취약하지만, 이 시스템은 고수온만 다룬다.
"""
